...
void AD7793_Init(void) {
   ...
   ui32reg_value = AD7793_ReadRegister(AD7793_REG_IO);
   Serial.print("IO register: ");
   Serial.println(ui32reg_value, HEX);
}
...
void AD7793_Reset(void) {
   SPI_Write(0, 0x03, 1);  // Write 4 bytes = 0xFF
...
float AD7793_ConvertToVolts(uint32_t u32adcValue) {
   ...
   // Vref = 1170 [mV]
   f32voltage = (1.17 * (float)(u32adcValue - 0x800000)) / ((float)0x800000);

...
HydraulicPumpController myPumps[ACTIVE_PUMPS] = {
    HydraulicPumpController("code04", outputGPIOs[0], 60000),
    HydraulicPumpController("code05", outputGPIOs[1], 60000),
   ...
   if (String(topic) == String(WiFi.getHostname()) + "/output") {
      ...
      if (messageTemp == "get all") {
         ...
         for (int indice = 0; indice < NUMBER_OUTPUTS; indice++) {
            ...
            port["state"] = digitalRead(myPumps[indice].getGpio()) == HIGH ? true : false;
         ...
         // serializeJson(jsonPayload, Serial);  // Para depuração
...
void initPumpConfiguration() {
   for (int indice = 0; indice < ACTIVE_PUMPS; indice++) {
      loadDriveTimes(*(myPumps[indice].getDriveTimesPointer()), "/driveTimes.bin");
   }

   Serial.println("Pump configuration initialized");
}
